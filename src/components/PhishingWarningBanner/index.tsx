...
import { Text, Flex, Box, CloseIcon, IconButton, useMatchBreakpoints } from '@respctclub/uikit'
...
const PhishingWarningBanner: React.FC = () => {
  ...
  const warningText = t("please make sure you're visiting https://app.respct.club - check the URL carefully.")
  const warningTextAsParts = warningText.split(/(https:\/\/app.respct.club)/g)
    ...
    <>
      ...
      {warningTextAsParts.map((text, i) => (
        <Text
          ...
          small
          as="span"
          bold={text === 'https://app.respct.club'}
          color={text === 'https://app.respct.club' ? '#FFFFFF' : '#BDC2C4'}
        >
          {text}
        </Text>
      ))}
