...
import { TokenAmount, Token, ChainId, Percent, JSBI } from '@respctclub/sdk'
...
describe('utils', () => {
  describe('#getBscScanLink', () => {
    it('correct for tx', () => {
      expect(getBscScanLink('abc', 'transaction', ChainId.BSCTESTNET)).toEqual('https://bscscan.com/tx/abc')
    })
    it('correct for token', () => {
      expect(getBscScanLink('abc', 'token', ChainId.BSCTESTNET)).toEqual('https://bscscan.com/token/abc')
    })
    it('correct for address', () => {
      expect(getBscScanLink('abc', 'address', ChainId.BSCTESTNET)).toEqual('https://bscscan.com/address/abc')
    })
    it('enum', () => {
  ...
  describe('#calculateSlippageAmount', () => {
    it('bounds are correct', () => {
      const tokenAmount = new TokenAmount(new Token(ChainId.BSCTESTNET, AddressZero, 0), '100')
